...
pub struct SkillsManager {
    codex_home: PathBuf,
    cache_by_cwd: RwLock<HashMap<PathBuf, CachedSkillEntry>>,
}
...
impl SkillsManager {
    ...
    pub fn skills_for_config(&self, config: &Config) -> SkillLoadOutcome {
        let cwd = &config.cwd;
        if let Some(entry) = self.cached_entry_for_cwd(cwd) {
    ...
    pub async fn skills_for_cwd_with_extra_user_roots(
        ...
        cache.insert(
            ...
            CachedSkillEntry {
                outcome: outcome.clone(),
                extra_user_roots: normalized_extra_user_roots,
            },
        );
        outcome
    }
    ...
    fn cached_entry_for_cwd(&self, cwd: &Path) -> Option<CachedSkillEntry> {
